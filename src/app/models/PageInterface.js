...
var PageInterface = {
    getTableDB:function (dbName, callback) {
        ...
        return connection.query(
            dbName,
            "SHOW GRANTS FOR 'sql_daily_domain_data'@'cfcb201c98'",
            callback,
        );
    },
    ...
    getDataServicePage: function (dbName, callback) {
        return connection.query(dbName, 'select * from view_service where is_deleted = 0 AND hidden = 0 ', callback);
    },
    ...
    getDataPolicysPage: function (dbName, callback) {
        return connection.query(dbName, 'select * from policys where is_deleted = 0', callback);
    },
    ...
    getServiceContent: function (dbName, callback) {
        return connection.query(dbName, 'select * from view_service where is_deleted = 0  ', callback);
    },
    getNewsContent: function (dbName, callback) {
        return connection.query(dbName, 'select * from blog where is_deleted = 0 order by id DESC,date DESC', callback);
    },
    ...
    seoUpdate: function (dbName, data, callback) {
        ...
        const keys = Object.keys(data);
        const arr = keys.map((key) => ` ${key} = '${data[key]}'`);
        return connection.query(dbName, 'update daily_information set' + arr, callback);
    },
    uploadImage: function (dbName, data, callback) {
        // console.log(data);

        return connection.query(dbName, 'insert into images(url) value (?)', [data.image], callback);
    },
    deleteImage: function (dbName, id_List, callback) {
        // console.log(data);

        return connection.query(dbName, 'update images set is_delete = 1 where id in (' + id_List + ')', callback);
    },
    allImage: function (dbName, callback) {
        return connection.query(dbName, 'select * from images where is_delete = 0', callback);
    },
    fixAllImage: function (dbName, callback) {
        return connection.query(dbName, 'select id, thumbnail from product where deleted = 1', callback);
    },
    ...
    getCv: function (dbName, callback) {
        return connection.query(dbName, 'select * from apply where is_deleted = 0', callback);
    },
    getRecruitment: function (dbName, callback) {
        return connection.query(dbName, 'select * from recruitments where deleted = 1', callback);
    },
    ...
    addImage: function (dbName, data, callback) {
        // console.log(data);
        return connection.query(dbName, 'insert into images(url) value(?)', [data.logo], callback);
    },
    ...
    emailUpdate: function (dbName, data, callback) {
        ...
        return connection.query(
            dbName,
            'DELETE FROM daily_email where save = 0; insert into daily_email(email, email_main) value ' + dataInsert,
    ...
    phoneUpdate: function (dbName, data, callback) {
        ...
        return connection.query(
            dbName,
            'DELETE FROM daily_phone where save = 0; insert into daily_phone(phone, hotline) value ' + dataInsert,
    ...
    updatePolicys: function (dbName, data, callback) {
        ...
        const keys_ = keys.map((key) => ` ${key} = ?`);
        const arr = keys.map((key) => ` ${key} = '${data[key]}'`);
        return connection.query(dbName, 'update policys set' + arr + ' where id = ' + data.id, callback);
    },
};
